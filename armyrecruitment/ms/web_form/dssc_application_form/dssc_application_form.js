frappe.ready(function() {
	...
	$('#page-dssc-application-form').before(header);
	$('#page-dssc-application-form').after(footer);
	...
	if(currentPage.current_section == 0 && currentPage.is_new == true) {
		...
		$validate_button.on('click', function() {
			...
			if(!nin || !date_of_birth || !discipline) {
                frappe.msgprint(__('Please enter NIN, Date of Birth and Discipline to proceed'));
                return;
            }
				...
				frappe.call({
					...
					callback: function(r) {
						...
						if(r.message) {
							...
							currentPage.set_df_property('discipline', 'read_only', true);
	...
	currentPage.on('date_of_birth', () => {
		...
		let ageLimits = DISCIPLINE_AGE_LIMITS[category];
		
		if (age < ageLimits.min || age > ageLimits.max) {
			frappe.msgprint(__(`For ${discipline}, age must be between ${ageLimits.min} and ${ageLimits.max} years`));
			currentPage.set_value('date_of_birth', '');
			return;
		}
